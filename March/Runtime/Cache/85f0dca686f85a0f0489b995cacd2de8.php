...
<div class="message">
	<div class="msg">
	<?php if(isset($message)): ?><span class="success">{$msgTitle}{$message}</span>
	<?php else: ?>
	<span class="error">{$msgTitle}{$error}</span><?php endif; ?>
	</div>
	<div class="tip">
	<?php if(isset($closeWin)): ?>页面将在 <span class="wait">{$waitSecond}</span> 秒后自动关闭，如果不想等待请点击 <a href="{$jumpUrl}">这里</a> 关闭
	<?php else: ?>
		页面将在 <span class="wait">{$waitSecond}</span> 秒后自动跳转，如果不想等待请点击 <a href="{$jumpUrl}">这里</a> 跳转<?php endif; ?>
	</div>
</div>
</body>
</html>